    ...
    'Host': 'kandian.wkandian.com'
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                const result = JSON.parse(data)
                if(result.status == 0){
                    console.log('今日收益总计:'+result.user.today_score)
                    console.log('当前金币总数:'+result.user.score)
                    console.log('折合人民币总数:'+result.user.money)
...
function do_getOrderList() {	
    return new Promise((resolve) => {
        ...
        const url = {
            url: `https://kandian.wkandian.com/withdraw/getOrderList?_=`+time,
            headers : {'Host': 'kandian.wkandian.com',
                'Referer':'https://kandian.wkandian.com/html/withdraw/order.html?'+zq_cookie1
            },
			body:zq_cookie1
        }
